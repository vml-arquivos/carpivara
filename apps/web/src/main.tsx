...
type AdminTab = 'overview' | 'users' | 'queries' | 'products' | 'payments' | 'audit' | 'lookup' | 'settings' | 'coupons' | 'affiliates' | 'organizations';
...
type Product = { id: string; name: string; description: string; creditCost: number; active?: boolean; slug?: string; features?: string[]; isFree?: boolean; featured?: boolean };
...
function App() {
  ...
  async function refresh(loadAdmin = false) {
    ...
    try {
      ...
      if (loadAdmin || isTeamRole(profile.user.role)) {
        ...
        const [summary, settings, users, payments, queries, auditEntries, catalog, coupons, affiliates, commissions, organizations] = await Promise.all([
        ...
        setAdmin(summary); setAdminSettings(settings); setAdminUsers(users); setAdminPayments(payments); setAdminQueries(queries); setAdminAudit(auditEntries); setAdminProducts(catalog); setAdminCoupons(coupons); setAdminAffiliates(affiliates); setAdminCommissions(commissions); setAdminOrganizations(organizations);
      }
    } catch (reason) {
  ...
  async function adminUpdateProduct(id: string, input: { name?: string; description?: string; creditCost?: number; active?: boolean }) {
    setLoading(true); setError('');
...
function AdminView({ summary, products, users, payments, queries, auditEntries, permissions, tab, setTab, currentUserId, loading, settings, coupons, affiliates, commissions, organizations, organizationMembers, onLoadOrganizationMembers, onUpsertOrganizationMember, onDeleteOrganizationMember, onUpdateUser, onDeleteUser, onAdjustWallet, onUpdateProduct, onLookup, onSaveSettings, onCreateCoupon, onUpdateCoupon, onDeleteCoupon, onCreateAffiliate, onUpdateAffiliate, onUpdateCommission, onCreateOrganization, onUpdateOrganization }: { summary: AdminSummary | null; products: Product[]; users: AdminUser[]; payments: AdminPayment[]; queries: AdminQuery[]; auditEntries: AdminAudit[]; permissions: string[]; tab: AdminTab; setTab: (tab: AdminTab) => void; currentUserId?: string; loading: boolean; settings: AdminSettings | null; coupons: AdminCoupon[]; affiliates: AdminAffiliate[]; commissions: AdminCommission[]; organizations: AdminOrganization[]; organizationMembers: AdminOrganizationMember[]; onLoadOrganizationMembers: (id: string) => Promise<void>; onUpsertOrganizationMember: (organizationId: string, input: { userId: string; role: AdminOrganizationMember['role'] }) => Promise<void>; onDeleteOrganizationMember: (organizationId: string, userId: string) => Promise<void>; onUpdateUser: (id: string, input: { role?: string; active?: boolean }) => Promise<void>; onDeleteUser: (id: string) => Promise<void>; onAdjustWallet: (id: string, input: { amount: number; description: string }) => Promise<void>; onUpdateProduct: (id: string, input: { name?: string; description?: string; creditCost?: number; active?: boolean }) => Promise<void>; onLookup: (input: { plate: string; productId: string }) => Promise<AdminLookup | null>; onSaveSettings: (input: Partial<AdminSettings['safe']>) => Promise<void>; onCreateCoupon: (input: Record<string, unknown>) => Promise<void>; onUpdateCoupon: (id: string, input: Record<string, unknown>) => Promise<void>; onDeleteCoupon: (id: string) => Promise<void>; onCreateAffiliate: (input: Record<string, unknown>) => Promise<void>; onUpdateAffiliate: (id: string, input: Record<string, unknown>) => Promise<void>; onUpdateCommission: (id: string, status: 'PAID' | 'CANCELLED') => Promise<void>; onCreateOrganization: (input: Record<string, unknown>) => Promise<void>; onUpdateOrganization: (id: string, input: Record<string, unknown>) => Promise<void> }) {
  ...
  const tabs: Array<[AdminTab, string, boolean]> = [['overview', 'Visão geral', canAudit], ['users', 'Usuários', canUsers], ['queries', 'Consultas', canAudit], ['products', 'Produtos', canPricing], ['payments', 'Pagamentos', canBilling], ['coupons', 'Cupons', canBilling], ['affiliates', 'Afiliados', canBilling], ['settings', 'Configurações', canProviders], ['organizations', 'Organizações', canSystem], ['audit', 'Auditoria', canAudit], ['lookup', 'Consulta interna', canLookup]];
  return <>
