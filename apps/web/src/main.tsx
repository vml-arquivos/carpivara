...
type AdminTab = 'overview' | 'users' | 'products' | 'payments' | 'audit' | 'lookup';
...
function App() {
  ...
  const canAdmin = Boolean(me?.permissions.some((permission) => ['VIEW_AUDIT', 'VIEW_SENSITIVE_DATA', 'MANAGE_USERS', 'MANAGE_PRICING', 'MANAGE_BILLING'].includes(permission)));
  ...
  function navigate(next: View) { setView(next); setError(''); if (next === 'admin') { setAdminTab('overview'); void refresh(true); } }
  ...
  return <div className="appShell"><aside className={adminMode ? 'sidebar adminSidebar' : 'sidebar'}><Brand /><div className="workspaceLabel"><span>{adminMode ? 'Painel administrativo' : 'Área do cliente'}</span><b>{adminMode ? 'Operação protegida' : 'Conta protegida'}</b></div><nav className="sideNav" aria-label={adminMode ? 'Navegação administrativa' : 'Navegação da plataforma'}>{adminMode ? <><button className="active adminNavPrimary" onClick={() => navigate('admin')}><i>◈</i> Centro de controle</button><button onClick={() => navigate('consult')}><i>↩</i> Voltar ao cliente</button></> : <><button className={view === 'consult' ? 'active' : ''} onClick={() => navigate('consult')}><i>⌁</i> Nova consulta</button><button className={view === 'history' ? 'active' : ''} onClick={() => navigate('history')}><i>◫</i> Histórico</button><button className={view === 'wallet' ? 'active' : ''} onClick={() => navigate('wallet')}><i>◇</i> Carteira</button><button className={view === 'settings' ? 'active' : ''} onClick={() => navigate('settings')}><i>◌</i> Perfil e segurança</button>{canAdmin && <button onClick={() => navigate('admin')}><i>◈</i> Administração</button>}</>}</nav><div className="sidePublicLinks" aria-label="Acessos públicos"><a href="/?site=1">Página inicial</a><a href="/fipe">Consulta FIPE grátis</a></div><div className="sideBottom"><div className="sideCredit"><span>{adminMode ? 'Carteira do cliente' : 'Saldo disponível'}</span><strong>{formatCredits(me?.balance ?? 0)} <small>créditos</small></strong></div><button className="logoutButton" onClick={logout}>Sair da conta</button></div></aside><main className="workspace"><header className={adminMode ? 'appHeader adminHeader' : 'appHeader'}><div><p className="kicker">{view === 'consult' ? 'Consulta veicular' : view === 'history' ? 'Histórico de consultas' : view === 'wallet' ? 'Carteira de créditos' : view === 'admin' ? 'Retaguarda BUSCARR' : 'Perfil e segurança'}</p><h1>{view === 'consult' ? 'Bom ter você por aqui, ' : ''}{view === 'consult' ? (me?.user.name?.split(' ')[0] ?? 'cliente') : view === 'history' ? 'Suas consultas, organizadas.' : view === 'wallet' ? 'Clareza em cada crédito.' : view === 'admin' ? 'Controle da operação.' : 'Perfil e segurança.'}</h1></div><div className="headerActions"><div className="headerQuickLinks" aria-label="Atalhos do site"><a href="/?site=1">Início</a><a href="/fipe">FIPE grátis</a></div><ThemeControl theme={theme} setTheme={setTheme} />{!adminMode && <button className="profileEditButton" type="button" onClick={() => navigate('settings')}>Editar usuário e senha</button>}{adminMode && <button className="profileEditButton adminReturnButton" type="button" onClick={() => navigate('consult')}>Voltar à conta</button>}<div className="profileBadge"><span>{me?.user.name?.slice(0, 1).toUpperCase()}</span><div><strong>{me?.user.name}</strong><small>{me?.user.role.replace('_', ' ')}</small></div></div></div></header>{toast && <div className="toast" role="status">{toast}<button onClick={() => setToast('')} aria-label="Fechar aviso">×</button></div>}{error && <div className="notice noticeError" role="alert">{error}<button onClick={() => setError('')} aria-label="Fechar erro">×</button></div>}{view === 'consult' && <ConsultView plate={plate} setPlate={setPlate} products={products} productId={productId} setProductId={setProductId} selectedProduct={selectedProduct} balance={me?.balance ?? 0} loading={loading} queryStage={queryStage} runQuery={runQuery} report={report} exportReport={exportReport} />}{view === 'history' && <HistoryView history={filteredHistory} filter={historyFilter} setFilter={setHistoryFilter} loading={loading} openSavedQuery={openSavedQuery} onRepeat={(item) => { setPlate(item.plate); setProductId(item.productId); setReport(null); navigate('consult'); setToast('Placa e produto preenchidos. Uma nova consulta consumirá créditos.'); }} />}{view === 'wallet' && <WalletView balance={me?.balance ?? 0} transactions={transactions} packages={creditPackages} products={products} loading={loading} startCheckout={startCheckout} />}{view === 'settings' && <SettingsView theme={theme} setTheme={setTheme} user={me?.user} profile={me?.profile} onSaveProfile={saveProfile} onChangePassword={changePassword} loading={loading} />}{view === 'admin' && canAdmin && <AdminView summary={admin} products={adminProducts} users={adminUsers} payments={adminPayments} queries={adminQueries} auditEntries={adminAudit} permissions={adminPermissions} tab={adminTab} setTab={setAdminTab} currentUserId={me?.user.id} loading={loading} onUpdateUser={adminUpdateUser} onDeleteUser={adminDeleteUser} onAdjustWallet={adminAdjustWallet} onUpdateProduct={adminUpdateProduct} onLookup={adminLookup} />}</main><nav className="mobileNav" aria-label="Navegação móvel"><button className={view === 'consult' ? 'active' : ''} onClick={() => navigate('consult')}>Consultar</button><button className={view === 'history' ? 'active' : ''} onClick={() => navigate('history')}>Histórico</button><button className={view === 'wallet' ? 'active' : ''} onClick={() => navigate('wallet')}>Carteira</button>{canAdmin && <button className={view === 'admin' ? 'active' : ''} onClick={() => navigate('admin')}>Admin</button>}<a href="/?site=1">Início</a><button onClick={logout}>Sair</button></nav></div>;
}
...
function AdminView({ summary, products, users, payments, queries, auditEntries, permissions, tab, setTab, currentUserId, loading, onUpdateUser, onDeleteUser, onAdjustWallet, onUpdateProduct, onLookup }: { summary: AdminSummary | null; products: Product[]; users: AdminUser[]; payments: AdminPayment[]; queries: AdminQuery[]; auditEntries: AdminAudit[]; permissions: string[]; tab: AdminTab; setTab: (tab: AdminTab) => void; currentUserId?: string; loading: boolean; onUpdateUser: (id: string, input: { role?: string; active?: boolean }) => Promise<void>; onDeleteUser: (id: string) => Promise<void>; onAdjustWallet: (id: string, input: { amount: number; description: string }) => Promise<void>; onUpdateProduct: (id: string, input: { name?: string; description?: string; creditCost?: number; active?: boolean }) => Promise<void>; onLookup: (input: { plate: string; productId: string }) => Promise<AdminLookup | null> }) {
  ...
  const tabs: Array<[AdminTab, string, boolean]> = [
    ['overview', 'Visão geral', canAudit], ['users', 'Usuários', canUsers], ['products', 'Produtos', canPricing], ['payments', 'Pagamentos', canBilling], ['audit', 'Auditoria', canAudit], ['lookup', 'Consulta interna', canLookup]
  ];
  return <>
