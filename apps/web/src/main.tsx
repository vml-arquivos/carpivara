...
function FipeView({ token = '', onAccess, onBack }: { token?: string; onAccess: () => void; onBack: () => void }) {
  ...
  async function consultFipe(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      if (!response.ok) throw new Error(body.message ?? 'Não foi possível consultar o valor FIPE.');
      setQuote(body);
    } catch (reason) { setError(reason instanceof Error ? reason.message : 'Não foi possível consultar o valor FIPE.'); }
  ...
  return <div className="fipePage"><header className="publicHeader"><Brand compact /><div className="fipeHeaderActions"><button className="textButton" onClick={onBack}>Voltar</button>{!token && <button className="primaryButton compact" onClick={onAccess}>Criar conta</button>}</div></header><main className="fipeMain"><section className="fipeHero"><div><p className="kicker">Consulta FIPE gratuita</p><h1>Descubra o valor médio antes de negociar.</h1><p>Informe a placa e encontre automaticamente o veículo para consultar o valor FIPE. O resultado é gratuito e aparece na hora.</p></div><div className="fipePromise"><strong>R$ 0</strong><span>sem cobrança</span><small>Consulta de valor médio. A FIPE não verifica débitos, gravame, roubo/furto, leilão ou sinistro.</small></div></section><section className="fipeFormCard"><div className="sectionHeading"><p className="kicker">Passo 1 de 2</p><h2>Como você quer consultar?</h2><p>Use a placa para identificar o veículo automaticamente ou selecione os dados manualmente.</p></div><div className="fipeModeTabs" role="tablist" aria-label="Modo de consulta"><button type="button" className={mode === 'plate' ? 'active' : ''} onClick={() => { setMode('plate'); setError(''); }} role="tab" aria-selected={mode === 'plate'}>Por placa</button><button type="button" className={mode === 'manual' ? 'active' : ''} onClick={() => { setMode('manual'); setError(''); }} role="tab" aria-selected={mode === 'manual'}>Escolher veículo</button></div><form onSubmit={consultFipe}>{mode === 'plate' ? <div className="fipePlatePanel"><label>Placa do veículo<input value={plate} onChange={(event) => setPlate(event.target.value.toUpperCase().replace(/[^A-Z0-9]/g, '').slice(0, 7))} placeholder="ABC1D23" maxLength={7} autoComplete="off" autoFocus /><small>Digite a placa sem espaços ou caracteres especiais.</small></label><div className="plateBenefit"><strong>Identificação automática</strong><span>O sistema encontra marca, modelo e ano para você.</span></div></div> : <div className="fipeFields"><label>Tipo<select value={vehicleType} onChange={(event) => setVehicleType(event.target.value as FipeQuote['vehicleType'])}><option value="cars">Carros</option><option value="motorcycles">Motos</option><option value="trucks">Caminhões</option></select></label><label>Marca<select value={brand?.code ?? ''} onChange={(event) => setBrand(brands.find((item) => item.code === event.target.value) ?? null)}><option value="">Selecione</option>{brands.map((item) => <option key={item.code} value={item.code}>{item.name}</option>)}</select></label><label>Modelo<select value={model?.code ?? ''} disabled={!brand} onChange={(event) => setModel(models.find((item) => item.code === event.target.value) ?? null)}><option value="">Selecione</option>{models.map((item) => <option key={item.code} value={item.code}>{item.name}</option>)}</select></label><label>Ano<select value={year?.code ?? ''} disabled={!model} onChange={(event) => setYear(years.find((item) => item.code === event.target.value) ?? null)}><option value="">Selecione</option>{years.map((item) => <option key={item.code} value={item.code}>{item.name}</option>)}</select></label></div>}{error && <div className="notice noticeError" role="alert">{error}</div>}<div className="fipeFormFooter"><span><strong>Grátis</strong> · sem consulta documental</span><button className="primaryButton" disabled={loading || (mode === 'plate' ? plate.length !== 7 : !brand || !model || !year)}>{loading ? 'Consultando...' : 'Consultar valor FIPE'} <span>→</span></button></div></form></section>{quote && <section className="fipeResultCard"><div className="fipeResultHeader"><div><p className="kicker">Resultado disponível</p><h2>{quote.brand.name} / {quote.model.name}</h2><p>{fipeTypeLabels[quote.vehicleType]} · {quote.year.name} · {quote.fuel || 'Combustível não informado'} · referência {quote.referenceMonth}</p>{quote.plate && <p>Placa consultada: <strong>{quote.plate}</strong></p>}</div><div className="fipeValue"><small>Valor FIPE vigente</small><strong>{quote.valueLabel}</strong><span>{quote.fipeCode}</span></div></div><div className="fipeReportMeta"><span>Documento <b>{quote.documentCode}</b></span><span>Hash <b>{quote.reportHash.slice(0, 18)}…</b></span></div><div className="fipeBlocks">{quote.blocks.map((block) => <div key={block.key} className={`fipeBlock ${block.state.toLowerCase()}`}><span>{block.state === 'FOUND' ? '✓' : '—'}</span><div><strong>{block.label}</strong><p>{block.message}</p></div></div>)}</div>{quote.estimatedNegotiation && <div className="fipeEstimate"><strong>Faixa informativa: {formatMoney(quote.estimatedNegotiation.minCents)} a {formatMoney(quote.estimatedNegotiation.maxCents)}</strong><p>{quote.estimatedNegotiation.disclaimer}</p></div>}<div className="fipeActions"><button className="secondaryButton" onClick={() => requireAccount('print')}>Imprimir dados</button><button className="secondaryButton" onClick={() => requireAccount('pdf')}>Gerar PDF</button><button className="primaryButton" onClick={() => requireAccount('save')} disabled={loading || saved}>{saved ? 'Salvo no histórico' : token ? 'Salvar no histórico' : 'Criar conta gratuita'} <span>→</span></button></div>{!token && <p className="fipeGateNotice">O valor FIPE é gratuito. Para imprimir, gerar PDF ou salvar, crie sua conta gratuita.</p>}</section>}{offers.filter((offer) => offer.id !== 'FIPE_FREE').length > 0 && <section className="fipeOffers"><div className="sectionHeading"><p className="kicker">Próximos níveis de segurança</p><h2>FIPE orienta. As próximas consultas aprofundam.</h2><p>Depois de consultar o valor, você pode escolher uma análise mais completa quando precisar.</p></div><div className="fipeOfferGrid">{offers.filter((offer) => offer.id !== 'FIPE_FREE').map((offer) => { const soon = offer.commercialStatus !== 'ACTIVE'; return <article key={offer.id} className={offer.featured ? 'featuredOffer' : ''}><span className="offerStatus">{soon ? 'Em breve' : `${formatCredits(offer.creditCost)} créditos`}</span><h3>{offer.name}</h3><p>{offer.description}</p><button className={soon ? 'secondaryButton' : 'primaryButton'} disabled={soon} onClick={onAccess}>{soon ? 'Em breve' : 'Continuar'}{!soon && <span>→</span>}</button></article>; })}</div></section>}</main><footer><strong>CARPIVARA</strong><span>FIPE gratuita para orientar sua decisão, sem confundir valor médio com situação documental.</span></footer></div>;
}
