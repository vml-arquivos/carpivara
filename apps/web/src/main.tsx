...
type AdminPackagePrice = { id: string; organizationId: string; packageId: string; packageSlug: string; packageName: string; basePriceCents: number; priceCents: number; active: boolean; startsAt?: string | null; endsAt?: string | null };
...
type CheckoutQuote = { packageSlug: string; packageName: string; credits: number; couponCode: string | null; affiliateCode: string | null; subtotalCents: number; discountCents: number; amountCents: number; paymentProviderConfigured: boolean; usageCountChangesOnlyAfterPaid: boolean };
...
function App() {
  ...
  const [adminPackagePrices, setAdminPackagePrices] = useState<Record<string, AdminPackagePrice[]>>({});
  ...
  async function refresh(loadAdmin = false) {
    if (!token) return;
    try {
      const [profile, productList, queries, wallet, packages, organizationContext] = await Promise.all([api<Me>('/me'), api<Product[]>('/query-products'), api<Query[]>('/queries'), api<Transaction[]>('/wallet/transactions'), api<CreditPackage[]>('/credit-packages'), api<OrganizationContext>('/organization/context')]);
      setMe(profile); setOrganization(organizationContext.organization); if (isTeamRole(profile.user.role)) { setView('admin'); if (!loadAdmin) setAdminTab(firstAdminTabFor(profile.permissions)); } setProducts(productList); setHistory(queries); setTransactions(wallet); setCreditPackages(packages);
  ...
  async function startCheckout(packageSlug: string, couponCode?: string, affiliateCode?: string) {
    setLoading(true); setError('');
    try {
      const order = await api<{ checkoutUrl: string }>('/payments/checkout', { method: 'POST', body: JSON.stringify({ packageSlug, ...(couponCode ? { couponCode } : {}), ...(affiliateCode ? { affiliateCode } : {}) }) });
  ...
  async function previewCheckout(packageSlug: string, couponCode?: string, affiliateCode?: string) {
    setQuoteLoading(true); setQuoteError(''); setCheckoutQuote(null);
    try {
      const quote = await api<CheckoutQuote>('/payments/quote', { method: 'POST', body: JSON.stringify({ packageSlug, ...(couponCode ? { couponCode } : {}), ...(affiliateCode ? { affiliateCode } : {}) }) });
      setCheckoutQuote(quote);
    } catch (reason) { setQuoteError(reason instanceof Error ? reason.message : 'Não foi possível validar o valor agora.'); }
  ...
  async function adminAdjustWallet(id: string, input: { amount: number; description: string }) {
    setLoading(true); setError('');
    try {
      const result = await api<{ balanceBefore?: number; balanceAfter?: number; balance: number }>(`/admin/users/${id}/wallet-adjustments`, { method: 'POST', body: JSON.stringify(input) });
      await refresh(true);
      setToast(`Carteira ajustada: ${formatCredits(result.balanceBefore ?? result.balance - input.amount)} → ${formatCredits(result.balanceAfter ?? result.balance)} créditos.`);
  ...
  async function adminLoadPackagePrices(organizationId: string) {
    try { const rows = await api<AdminPackagePrice[]>(`/admin/organizations/${organizationId}/credit-package-prices`); setAdminPackagePrices((current) => ({ ...current, [organizationId]: rows })); }
    catch (reason) { setError(reason instanceof Error ? reason.message : 'Não foi possível carregar os preços da organização.'); }
  }
  async function adminSavePackagePrice(organizationId: string, input: { packageSlug: string; priceCents: number; active: boolean; startsAt?: string | null; endsAt?: string | null }) {
    setLoading(true); setError('');
    try { await api(`/admin/organizations/${organizationId}/credit-package-prices`, { method: 'PUT', body: JSON.stringify(input) }); await adminLoadPackagePrices(organizationId); setToast('Preço negociado salvo.'); }
    catch (reason) { setError(reason instanceof Error ? reason.message : 'Não foi possível salvar o preço negociado.'); }
    finally { setLoading(false); }
  }
  async function adminDisablePackagePrice(organizationId: string, packageId: string) {
    setLoading(true); setError('');
    try { await api(`/admin/organizations/${organizationId}/credit-package-prices/${packageId}`, { method: 'DELETE' }); await adminLoadPackagePrices(organizationId); setToast('Preço negociado desativado.'); }
    catch (reason) { setError(reason instanceof Error ? reason.message : 'Não foi possível desativar o preço negociado.'); }
    finally { setLoading(false); }
  }
...
function WalletView({ balance, transactions, packages, products, loading, startCheckout, previewCheckout, quote, quoteLoading, quoteError, affiliate, onActivateAffiliate }: { balance: number; transactions: Transaction[]; packages: CreditPackage[]; products: Product[]; loading: boolean; startCheckout: (slug: string, couponCode?: string, affiliateCode?: string) => void; previewCheckout: (slug: string, couponCode?: string, affiliateCode?: string) => Promise<void>; quote: CheckoutQuote | null; quoteLoading: boolean; quoteError: string; affiliate: AffiliateSelf | null; onActivateAffiliate: () => Promise<void> }) {
  ...
  const normalizedCoupon = couponCode.trim() || null;
  const normalizedAffiliate = affiliateCode.trim() || null;
  function quoteMatches(pack: CreditPackage) {
    return Boolean(quote && quote.packageSlug === pack.slug && quote.couponCode === normalizedCoupon && quote.affiliateCode === normalizedAffiliate);
  }
  async function copyAffiliateLink() {
    if (!shareLink) return;
    try { await navigator.clipboard.writeText(shareLink); setLinkCopied(true); window.setTimeout(() => setLinkCopied(false), 2200); }
    catch { setLinkCopied(false); }
  }
  return <><section className="walletHero"><div><p className="kicker">Carteira BUSCARR</p><h2>{formatCredits(balance)} <small>créditos disponíveis</small></h2><p>Todo movimento fica registrado. Você escolhe um pacote, conclui o pagamento em checkout seguro e os créditos são liberados somente após a confirmação financeira.</p></div></section><section className="contentCard creditStore"><div className="listHeader"><div><p className="kicker">Comprar créditos</p><h2>Escolha o pacote ideal</h2><p>Pix e cartão são processados no checkout seguro do parceiro de pagamentos.</p><div className="notice noticeError" role="status">E-mails transacionais ainda não estão configurados. A recuperação de senha e o recibo por e-mail não serão enviados neste ambiente; acompanhe pagamentos e créditos pela Carteira e pelo histórico.</div></div></div><div className="checkoutOptions"><div><strong>Condições comerciais opcionais</strong><p>Se você recebeu um cupom ou indicação, informe antes de escolher o pacote. O valor final é validado no servidor, sem criar pedido ou reservar uso.</p></div><div className="formGrid"><label>Cupom de desconto<input value={couponCode} onChange={(event) => { setCouponCode(event.target.value.toUpperCase().replace(/\s/g, '')); }} placeholder="EX.: BUSCARR10" maxLength={40} /></label><label>Código de indicação<input value={affiliateCode} onChange={(event) => { setAffiliateCode(event.target.value.toUpperCase().replace(/\s/g, '')); }} placeholder="Opcional" maxLength={40} /></label></div>{quoteError && <div className="notice noticeError" role="alert">{quoteError}</div>}</div><div className="creditPackageGrid">{packages.map((pack) => { const matches = quoteMatches(pack); return <article key={pack.slug}><span>{formatCredits(pack.credits)} créditos</span><h3>{pack.name}</h3><p>{pack.description}</p><strong>{formatMoney(pack.priceCents)}</strong>{matches && quote && <div className="checkoutQuote" role="status" aria-live="polite"><span>Prévia validada no servidor</span><div><small>Subtotal</small><b>{formatMoney(quote.subtotalCents)}</b></div><div><small>Desconto {quote.couponCode ? `· ${quote.couponCode}` : ''}</small><b className="quoteDiscount">− {formatMoney(quote.discountCents)}</b></div><div><small>Total antes do checkout</small><b>{formatMoney(quote.amountCents)}</b></div><p>{quote.paymentProviderConfigured ? 'O provedor está pronto para receber o pedido.' : 'O provedor de pagamento ainda não está configurado; nenhum pedido foi criado.'}</p><small>{quote.usageCountChangesOnlyAfterPaid ? 'O contador de uso só muda após pagamento confirmado.' : ''}</small></div>}<button className="primaryButton" disabled={loading || quoteLoading} onClick={() => matches ? void startCheckout(pack.slug, normalizedCoupon ?? undefined, normalizedAffiliate ?? undefined) : void previewCheckout(pack.slug, normalizedCoupon ?? undefined, normalizedAffiliate ?? undefined)}>{matches ? 'Continuar para checkout' : quoteLoading ? 'Validando valor...' : 'Ver valor com condições'} <span>→</span></button></article>; })}</div></section>{!affiliateProfile ? <section className="contentCard affiliateCard"><div><p className="kicker">Indique e acompanhe</p><h2>Ative seu link de afiliado</h2><p>Compartilhe um link BUSCARR e acompanhe comissões pendentes. O pagamento das comissões é feito manualmente pela operação.</p></div><button className="secondaryButton" disabled={loading} onClick={() => void onActivateAffiliate()}>Ativar meu link</button></section> : <section className="contentCard affiliateCard"><div><p className="kicker">Seu canal de indicação</p><h2>{affiliateProfile.code}</h2><p>Comissões confirmadas: {formatMoney(affiliate?.totals?.paidCents ?? 0)} · pendentes: {formatMoney(affiliate?.totals?.pendingCents ?? 0)}</p><div className="affiliateShareBox"><code>{shareLink}</code><button type="button" className="tableButton" onClick={() => void copyAffiliateLink()}>{linkCopied ? 'Link copiado' : 'Copiar link'}</button></div><small>{formatCredits(affiliate?.totals?.referredUsers ?? 0)} pessoa(s) entraram pelo seu link.</small></div><span className="status status-success">Afiliado ativo</span></section>}<section className="contentCard planComposition"><div className="listHeader"><div><p className="kicker">Composição dos planos</p><h2>Saiba o que seus créditos permitem consultar.</h2><p>O custo de cada consulta aparece junto da cobertura correspondente. Itens em validação com fornecedor permanecem sinalizados.</p></div></div><div className="planCompositionGrid">{matrixPlans.map((plan) => { const product = products.find((item) => item.id === plan.id); const included = matrixFeatures.filter(([key]) => plan.cells[key] === 'included').map(([, label]) => label); const pending = matrixFeatures.filter(([key]) => plan.cells[key] === 'pending').map(([, label]) => label); return <article className={plan.disabled ? 'planDisabled' : plan.featured ? 'featuredComposition' : ''} key={plan.id}><div className="compositionTop"><span>{product ? `${formatCredits(product.creditCost)} créditos` : 'Em breve'}</span>{plan.featured && <b>Mais escolhido</b>}</div><h3>{plan.name}</h3><p>{plan.description}</p><div className="compositionList">{included.map((item) => <span className="included" key={item}>✓ {item}</span>)}{pending.map((item) => <span className="pending" key={item}>⚠ {item} · em validação</span>)}</div>{!plan.disabled && product && <small>Selecione este plano ao iniciar uma nova consulta.</small>}</article>; })}</div></section><section className="contentCard"><div className="listHeader"><div><h2>Movimentações</h2><p>Compras, consultas e estornos registrados em ordem cronológica.</p></div></div><div className="historyTable transactions"><div className="tableHead"><span>Movimento</span><span>Descrição</span><span>Data</span><span>Saldo após</span></div>{transactions.length === 0 ? <div className="emptyList"><strong>Sua carteira ainda não teve movimentações.</strong><p>Quando um pagamento for confirmado ou uma consulta for realizada, o histórico aparecerá aqui.</p></div> : transactions.map((item) => <div className="tableRow" key={item.id}><strong className={item.amount > 0 ? 'creditAmount' : 'debitAmount'}>{item.amount > 0 ? '+' : ''}{formatCredits(item.amount)} cr.</strong><span>{item.description}</span><span>{formatDate(item.createdAt)}</span><span>{formatCredits(item.balanceAfter)} cr.</span></div>)}</div></section></>;
}
...
function AdminView({ summary, products, users, payments, queries, auditEntries, permissions, tab, setTab, currentUserId, loading, settings, coupons, affiliates, commissions, organizations, organizationMembers, onLoadOrganizationMembers, onUpsertOrganizationMember, onDeleteOrganizationMember, onUpdateUser, onDeleteUser, onAdjustWallet, onUpdateProduct, onLookup, onSaveSettings, onCreateCoupon, onUpdateCoupon, onDeleteCoupon, onCreateAffiliate, onUpdateAffiliate, onUpdateCommission, onCreateOrganization, onUpdateOrganization, creditPackages, contacts, packagePrices, templates, onCreateProduct, onCreateTemplate, onLoadTemplates, onPublishTemplate, onLoadPackagePrices, onSavePackagePrice, onDisablePackagePrice, onUpdateContact, onRunRetention }: { summary: AdminSummary | null; products: Product[]; users: AdminUser[]; payments: AdminPayment[]; queries: AdminQuery[]; auditEntries: AdminAudit[]; permissions: string[]; tab: AdminTab; setTab: (tab: AdminTab) => void; currentUserId?: string; loading: boolean; settings: AdminSettings | null; coupons: AdminCoupon[]; affiliates: AdminAffiliate[]; commissions: AdminCommission[]; organizations: AdminOrganization[]; organizationMembers: AdminOrganizationMember[]; creditPackages: CreditPackage[]; contacts: ContactMessage[]; packagePrices: Record<string, AdminPackagePrice[]>; templates: Record<string, AdminReportTemplate[]>; onLoadOrganizationMembers: (id: string) => Promise<void>; onUpsertOrganizationMember: (organizationId: string, input: { userId: string; role: AdminOrganizationMember['role'] }) => Promise<void>; onDeleteOrganizationMember: (organizationId: string, userId: string) => Promise<void>; onUpdateUser: (id: string, input: { role?: string; active?: boolean }) => Promise<void>; onDeleteUser: (id: string) => Promise<void>; onAdjustWallet: (id: string, input: { amount: number; description: string }) => Promise<void>; onUpdateProduct: (id: string, input: Record<string, unknown>) => Promise<void>; onCreateProduct: (input: Record<string, unknown>) => Promise<void>; onCreateTemplate: (productId: string, input: { name: string; status: 'DRAFT' | 'PUBLISHED'; config: ReportTemplateConfig }) => Promise<void>; onLoadTemplates: (productId: string) => Promise<void>; onPublishTemplate: (id: string, productId: string) => Promise<void>; onLoadPackagePrices: (organizationId: string) => Promise<void>; onSavePackagePrice: (organizationId: string, input: { packageSlug: string; priceCents: number; active: boolean; startsAt?: string | null; endsAt?: string | null }) => Promise<void>; onDisablePackagePrice: (organizationId: string, packageId: string) => Promise<void>; onUpdateContact: (id: string, status: ContactMessage['status']) => Promise<void>; onRunRetention: (input: { olderThanDays: number; execute: boolean }) => Promise<void>; onLookup: (input: { plate: string; productId: string }) => Promise<AdminLookup | null>; onSaveSettings: (input: Partial<AdminSettings['safe']>) => Promise<void>; onCreateCoupon: (input: Record<string, unknown>) => Promise<void>; onUpdateCoupon: (id: string, input: Record<string, unknown>) => Promise<void>; onDeleteCoupon: (id: string) => Promise<void>; onCreateAffiliate: (input: Record<string, unknown>) => Promise<void>; onUpdateAffiliate: (id: string, input: Record<string, unknown>) => Promise<void>; onUpdateCommission: (id: string, status: 'PAID' | 'CANCELLED') => Promise<void>; onCreateOrganization: (input: Record<string, unknown>) => Promise<void>; onUpdateOrganization: (id: string, input: Record<string, unknown>) => Promise<void> }) {
  ...
  const [productForm, setProductForm] = useState({ id: '', slug: '', name: '', description: '', creditCost: '1', referencePriceCents: '', features: '', source: '', coverage: '', commercialStatus: 'ACTIVE' });
  ...
  const [priceForm, setPriceForm] = useState({ packageSlug: '', priceCents: '', active: true, startsAt: '', endsAt: '' });
  ...
  async function adjustWallet(event: FormEvent<HTMLFormElement>) { event.preventDefault(); if (!selectedUser) return; const form = new FormData(event.currentTarget); const amountValue = Number(form.get('amount') ?? 0); const description = String(form.get('description') ?? ''); if (!Number.isInteger(amountValue) || amountValue === 0 || description.trim().length < 8) return; await onAdjustWallet(selectedUser.id, { amount: amountValue, description }); event.currentTarget.reset(); }
  ...
  async function createProduct(event: FormEvent<HTMLFormElement>) { event.preventDefault(); await onCreateProduct({ id: productForm.id.toUpperCase(), slug: productForm.slug.toLowerCase(), name: productForm.name, description: productForm.description, creditCost: Number(productForm.creditCost), referencePriceCents: productForm.referencePriceCents ? Number(productForm.referencePriceCents) : null, features: productForm.features.split(/\n|,/).map((item) => item.trim()).filter(Boolean), source: productForm.source || null, coverage: productForm.coverage || null, commercialStatus: productForm.commercialStatus, active: true, isFree: false, featured: false, displayOrder: products.length }); setProductForm({ id: '', slug: '', name: '', description: '', creditCost: '1', referencePriceCents: '', features: '', source: '', coverage: '', commercialStatus: 'ACTIVE' }); }
  ...
  async function savePackagePrice(event: FormEvent<HTMLFormElement>) { event.preventDefault(); if (!selectedOrganization || !priceForm.packageSlug) return; await onSavePackagePrice(selectedOrganization.id, { packageSlug: priceForm.packageSlug, priceCents: Number(priceForm.priceCents), active: priceForm.active, startsAt: priceForm.startsAt || null, endsAt: priceForm.endsAt || null }); }
