...
type Product = { id: string; name: string; description: string; creditCost: number; slug?: string; features?: string[]; isFree?: boolean; source?: string; coverage?: string; featured?: boolean };
type FipeItem = { code: string; name: string };
type FipeOffer = { id: string; name: string; description: string; creditCost: number; features?: string[]; source?: string; coverage?: string; commercialStatus?: string; featured?: boolean };
type FipeQuote = { documentCode: string; reportHash: string; provider: string; source: string; consultedAt: string; referenceMonth: string; vehicleType: 'cars' | 'motorcycles' | 'trucks'; brand: FipeItem; model: FipeItem; year: FipeItem; fuel?: string; fipeCode: string; valueCents: number; valueLabel: string; estimatedNegotiation?: { minCents: number; maxCents: number; disclaimer: string }; blocks: Array<{ key: string; label: string; state: string; message: string }>; plate?: string };
...
function FipeView({ token = '', onAccess, onBack }: { token?: string; onAccess: () => void; onBack: () => void }) {
  ...
  const [brands, setBrands] = useState<FipeItem[]>([]); const [models, setModels] = useState<FipeItem[]>([]); const [years, setYears] = useState<FipeItem[]>([]);
  const [brand, setBrand] = useState<FipeItem | null>(null); const [model, setModel] = useState<FipeItem | null>(null); const [year, setYear] = useState<FipeItem | null>(null);
  const [quote, setQuote] = useState<FipeQuote | null>(null); const [offers, setOffers] = useState<FipeOffer[]>([]); const [plate, setPlate] = useState(''); const [loading, setLoading] = useState(false); const [error, setError] = useState(''); const [saved, setSaved] = useState(false);
  const headers: Record<string, string> = token ? { Authorization: `Bearer ${token}` } : {};
  async function loadOptions(path: string): Promise<FipeItem[]> { const response = await fetch(`${API}${path}`, { headers }); const body = await response.json() as { brands?: FipeItem[]; models?: FipeItem[]; years?: FipeItem[]; message?: string }; if (!response.ok) throw new Error(body.message ?? 'Não foi possível carregar a Tabela FIPE.'); return body.brands ?? body.models ?? body.years ?? []; }
  useEffect(() => { setBrand(null); setModel(null); setYear(null); setModels([]); setYears([]); setError(''); void loadOptions(`/fipe/brands?vehicleType=${vehicleType}`).then(setBrands).catch((reason) => setError(reason instanceof Error ? reason.message : 'Não foi possível carregar as marcas.')); }, [vehicleType]);
  ...
  useEffect(() => { if (!brand) return; setModel(null); setYear(null); setYears([]); void loadOptions(`/fipe/models?vehicleType=${vehicleType}&brandCode=${encodeURIComponent(brand.code)}`).then(setModels).catch((reason) => setError(reason instanceof Error ? reason.message : 'Não foi possível carregar os modelos.')); }, [brand, vehicleType]);
  useEffect(() => { if (!brand || !model) return; setYear(null); void loadOptions(`/fipe/years?vehicleType=${vehicleType}&brandCode=${encodeURIComponent(brand.code)}&modelCode=${encodeURIComponent(model.code)}`).then(setYears).catch((reason) => setError(reason instanceof Error ? reason.message : 'Não foi possível carregar os anos.')); }, [brand, model, vehicleType]);
  async function consultFipe(event: FormEvent<HTMLFormElement>) { event.preventDefault(); if (!brand || !model || !year) { setError('Escolha veículo, marca, modelo e ano para consultar.'); return; } setLoading(true); setError(''); setQuote(null); setSaved(false); try { const response = await fetch(`${API}/fipe/quote`, { method: 'POST', headers: { 'Content-Type': 'application/json', ...headers }, body: JSON.stringify({ vehicleType, brand, model, year, plate: plate || undefined }) }); const body = await response.json() as FipeQuote & { message?: string }; if (!response.ok) throw new Error(body.message ?? 'Não foi possível consultar a FIPE.'); setQuote(body); } catch (reason) { setError(reason instanceof Error ? reason.message : 'Não foi possível consultar a FIPE.'); } finally { setLoading(false); } }
  async function saveReport() { if (!quote || !token) return onAccess(); setLoading(true); try { const response = await fetch(`${API}/fipe/quotes/${quote.documentCode}/save`, { method: 'POST', headers: { 'Content-Type': 'application/json', ...headers } }); const body = await response.json() as { message?: string }; if (!response.ok) throw new Error(body.message ?? 'Não foi possível salvar o relatório.'); setSaved(true); } catch (reason) { setError(reason instanceof Error ? reason.message : 'Não foi possível salvar o relatório.'); } finally { setLoading(false); } }
  return <div className="fipePage"><header className="publicHeader"><Brand compact /><div className="fipeHeaderActions"><span className="sourcePill">Fonte: FIPE mensal</span><button className="textButton" onClick={onBack}>Voltar</button>{!token && <button className="primaryButton compact" onClick={onAccess}>Criar conta</button>}</div></header><main className="fipeMain"><section className="fipeHero"><div><p className="kicker">Consulta FIPE gratuita</p><h1>Descubra o valor médio antes de negociar.</h1><p>Selecione o veículo e receba a referência FIPE vigente sem consumir créditos. Gere um relatório, imprima e valide o documento publicamente.</p></div><div className="fipePromise"><strong>R$ 0</strong><span>sem cobrança</span><small>Consulta de valor médio. A FIPE não verifica débitos, gravame, roubo/furto, leilão ou sinistro.</small></div></section><section className="fipeFormCard"><div className="sectionHeading"><p className="kicker">Passo 1 de 2</p><h2>Escolha o veículo</h2><p>A fonte é consultada mensalmente; os dados abaixo são apenas para encontrar a versão correta.</p></div><form onSubmit={consultFipe}><div className="fipeFields"><label>Tipo<select value={vehicleType} onChange={(event) => setVehicleType(event.target.value as FipeQuote['vehicleType'])}><option value="cars">Carros</option><option value="motorcycles">Motos</option><option value="trucks">Caminhões</option></select></label><label>Marca<select value={brand?.code ?? ''} onChange={(event) => setBrand(brands.find((item) => item.code === event.target.value) ?? null)}><option value="">Selecione</option>{brands.map((item) => <option key={item.code} value={item.code}>{item.name}</option>)}</select></label><label>Modelo<select value={model?.code ?? ''} disabled={!brand} onChange={(event) => setModel(models.find((item) => item.code === event.target.value) ?? null)}><option value="">Selecione</option>{models.map((item) => <option key={item.code} value={item.code}>{item.name}</option>)}</select></label><label>Ano<select value={year?.code ?? ''} disabled={!model} onChange={(event) => setYear(years.find((item) => item.code === event.target.value) ?? null)}><option value="">Selecione</option>{years.map((item) => <option key={item.code} value={item.code}>{item.name}</option>)}</select></label><label>Placa (opcional)<input value={plate} onChange={(event) => setPlate(event.target.value.toUpperCase().replace(/[^A-Z0-9]/g, '').slice(0, 7))} placeholder="Para identificar o relatório" maxLength={7} /></label></div>{error && <div className="notice noticeError" role="alert">{error}</div>}<div className="fipeFormFooter"><span><strong>Grátis</strong> · sem consulta documental</span><button className="primaryButton" disabled={loading || !brand || !model || !year}>{loading ? 'Consultando...' : 'Consultar valor FIPE'} <span>→</span></button></div></form></section>{quote && <section className="fipeResultCard"><div className="fipeResultHeader"><div><p className="kicker">Resultado verificado</p><h2>{quote.brand.name} / {quote.model.name}</h2><p>{fipeTypeLabels[quote.vehicleType]} · {quote.year.name} · {quote.fuel || 'Combustível não informado'} · referência {quote.referenceMonth}</p></div><div className="fipeValue"><small>Valor FIPE vigente</small><strong>{quote.valueLabel}</strong><span>{quote.fipeCode}</span></div></div><div className="fipeReportMeta"><span>Documento <b>{quote.documentCode}</b></span><span>Hash <b>{quote.reportHash.slice(0, 18)}…</b></span><span>Fonte <b>{quote.source}</b></span></div><div className="fipeBlocks">{quote.blocks.map((block) => <div key={block.key} className={`fipeBlock ${block.state.toLowerCase()}`}><span>{block.state === 'FOUND' ? '✓' : '—'}</span><div><strong>{block.label}</strong><p>{block.message}</p></div></div>)}</div>{quote.estimatedNegotiation && <div className="fipeEstimate"><strong>Faixa informativa: {formatMoney(quote.estimatedNegotiation.minCents)} a {formatMoney(quote.estimatedNegotiation.maxCents)}</strong><p>{quote.estimatedNegotiation.disclaimer}</p></div>}<div className="fipeActions"><a className="secondaryButton" href={`${API}/fipe/reports/${quote.documentCode}/print`} target="_blank" rel="noreferrer">Imprimir</a><a className="secondaryButton" href={`${API}/fipe/reports/${quote.documentCode}/pdf`}>Baixar PDF</a><button className="primaryButton" onClick={() => void saveReport()} disabled={loading || saved}>{saved ? 'Salvo no histórico' : token ? 'Salvar no histórico' : 'Criar conta e salvar'} <span>→</span></button></div></section>}{offers.filter((offer) => offer.id !== 'FIPE_FREE').length > 0 && <section className="fipeOffers"><div className="sectionHeading"><p className="kicker">Próximos níveis de segurança</p><h2>FIPE orienta. As próximas consultas aprofundam.</h2><p>Você decide o próximo passo. Produtos sem fonte contratada aparecem como “em breve” e não podem ser comprados.</p></div><div className="fipeOfferGrid">{offers.filter((offer) => offer.id !== 'FIPE_FREE').map((offer) => { const soon = offer.commercialStatus !== 'ACTIVE'; return <article key={offer.id} className={offer.featured ? 'featuredOffer' : ''}><span className="offerStatus">{soon ? 'Em breve' : `${formatCredits(offer.creditCost)} créditos`}</span><h3>{offer.name}</h3><p>{offer.description}</p><small>{offer.coverage ?? 'Cobertura conforme fonte habilitada.'}</small><button className={soon ? 'secondaryButton' : 'primaryButton'} disabled={soon} onClick={onAccess}>{soon ? 'Aguardando fonte' : 'Continuar' }{!soon && <span>→</span>}</button></article>; })}</div></section>}</main><footer><strong>CARPIVARA</strong><span>FIPE gratuita para orientar sua decisão, sem confundir valor médio com situação documental.</span></footer></div>;
}

function ValidationPage({ code }: { code: string }) { const [result, setResult] = useState<{ authentic: boolean; status?: string; reportKind?: string; provider?: string; documentCode?: string; createdAt?: string; hash?: string; fipeReferenceMonth?: string } | null>(null); useEffect(() => { void fetch(`${API}/validar-relatorio/${encodeURIComponent(code)}`).then((response) => response.json()).then(setResult).catch(() => setResult({ authentic: false, status: 'ERROR' })); }, [code]); return <div className="validationPage"><Brand /><section className="validationCard"><p className="kicker">Validação pública</p>{!result ? <h1>Verificando documento…</h1> : result.authentic ? <><div className="validationIcon">✓</div><h1>Relatório autêntico</h1><p>O documento <strong>{result.documentCode}</strong> foi encontrado na base de validação da CARPIVARA.</p><div className="validationRows"><span>Status<strong>{result.status}</strong></span><span>Fonte<strong>{result.provider}</strong></span><span>Referência FIPE<strong>{result.fipeReferenceMonth ?? '—'}</strong></span><span>Hash<strong>{result.hash?.slice(0, 24)}…</strong></span></div><a className="primaryButton" href="/fipe">Consultar FIPE grátis <span>→</span></a></> : <><div className="validationIcon invalid">!</div><h1>Documento não encontrado</h1><p>O código informado não corresponde a um relatório público válido.</p><a className="secondaryButton" href="/fipe">Voltar à consulta FIPE</a></>}</section></div>; }
...
function App() {
  ...
  if (!token) {
    ...
    return publicPage === 'landing' ? <Landing theme={theme} setTheme={setTheme} onAccess={(mode) => { setAuthMode(mode); setError(''); setPublicPage('auth'); }} /> : <AuthScreen onAuthenticated={setToken} onBack={() => setPublicPage('landing')} externalError={error} initialMode={authMode} />;
  }
