...
type AdminTab = 'overview' | 'users' | 'queries' | 'products' | 'payments' | 'audit' | 'lookup';
...
function App() {
  ...
  async function refresh(loadAdmin = false) {
    ...
    try {
      ...
      setMe(profile); if (isTeamRole(profile.user.role)) { setView('admin'); setAdminTab(firstAdminTabFor(profile.permissions)); } setProducts(productList); setHistory(queries); setTransactions(wallet); setCreditPackages(packages);
      ...
      if (loadAdmin || isTeamRole(profile.user.role)) {
        ...
        const [summary, users, payments, queries, auditEntries, catalog] = await Promise.all([
        ...
        setAdmin(summary); setAdminUsers(users); setAdminPayments(payments); setAdminQueries(queries); setAdminAudit(auditEntries); setAdminProducts(catalog);
      }
    } catch (reason) {
  ...
  const canAdmin = Boolean(canViewAudit || canLookup || canManageUsers || canManagePricing || canManageBilling);
...
function AdminView({ summary, products, users, payments, queries, auditEntries, permissions, tab, setTab, currentUserId, loading, onUpdateUser, onDeleteUser, onAdjustWallet, onUpdateProduct, onLookup }: { summary: AdminSummary | null; products: Product[]; users: AdminUser[]; payments: AdminPayment[]; queries: AdminQuery[]; auditEntries: AdminAudit[]; permissions: string[]; tab: AdminTab; setTab: (tab: AdminTab) => void; currentUserId?: string; loading: boolean; onUpdateUser: (id: string, input: { role?: string; active?: boolean }) => Promise<void>; onDeleteUser: (id: string) => Promise<void>; onAdjustWallet: (id: string, input: { amount: number; description: string }) => Promise<void>; onUpdateProduct: (id: string, input: { name?: string; description?: string; creditCost?: number; active?: boolean }) => Promise<void>; onLookup: (input: { plate: string; productId: string }) => Promise<AdminLookup | null> }) {
  ...
  async function saveUser(event: FormEvent<HTMLFormElement>) {
    ...
    const form = new FormData(event.currentTarget);
    await onUpdateUser(selectedUser.id, { role: String(form.get('role') ?? selectedUser.role), active: form.get('active') === 'on' });
  }
  async function adjustWallet(event: FormEvent<HTMLFormElement>) {
    event.preventDefault(); if (!selectedUser) return;
    const form = new FormData(event.currentTarget); const amountValue = Number(form.get('amount') ?? 0); const description = String(form.get('description') ?? '');
    if (!Number.isInteger(amountValue) || amountValue === 0 || description.trim().length < 8) return;
    await onAdjustWallet(selectedUser.id, { amount: amountValue, description });
    event.currentTarget.reset();
  }
  async function deleteUser() {
    if (!selectedUser || selectedUser.id === currentUserId) return;
    if (!window.confirm(`Remover a conta de ${selectedUser.name}? As sessões serão revogadas.`)) return;
    await onDeleteUser(selectedUser.id); setSelectedUserId(null);
  }
  async function saveProduct(event: FormEvent<HTMLFormElement>) {
    event.preventDefault(); if (!selectedProduct) return;
    const form = new FormData(event.currentTarget); const creditCost = Number(form.get('creditCost') ?? 0);
    await onUpdateProduct(selectedProduct.id, { name: String(form.get('name') ?? ''), description: String(form.get('description') ?? ''), creditCost, active: form.get('active') === 'on' });
    setSelectedProductId(null);
  }
  async function submitLookup(event: FormEvent<HTMLFormElement>) {
    event.preventDefault(); setLookupResult(null);
    const result = await onLookup({ plate: lookupPlate, productId: lookupProductId });
    if (result) setLookupResult(result);
  }
  const availableProducts = products.filter((product) => product.active !== false);
  const tabs: Array<[AdminTab, string, boolean]> = [
    ['overview', 'Visão geral', canAudit], ['users', 'Usuários', canUsers], ['queries', 'Consultas', canAudit], ['products', 'Produtos', canPricing], ['payments', 'Pagamentos', canBilling], ['audit', 'Auditoria', canAudit], ['lookup', 'Consulta interna', canLookup]
  ];
  return <>
    <section className="adminIdentity contentCard"><div><p className="kicker">Ambiente restrito · operação</p><h2>Painel administrativo BUSCARR</h2><p>Esta área é separada do dashboard do cliente. Ações de suporte, carteira e catálogo ficam protegidas por permissão e registradas quando aplicável.</p></div><span className="adminIdentityBadge">Sessão {permissions.includes('ADMIN_SYSTEM') ? 'SUPER ADMIN' : 'ADMINISTRATIVA'}</span></section>
