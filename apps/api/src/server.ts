...
app.get('/health',async(_req,res)=>{
  try { await pool.query('SELECT 1'); res.json({ok:true,app:env.APP_NAME,provider:env.DATA_PROVIDER,database:'ok'}); }
  catch { res.status(503).json({ok:false,database:'unavailable'}); }
});

const api=express.Router();
api.post('/auth/register',async(req,res)=>{
  const parsed=z.object({name:z.string().trim().min(2).max(120),email:z.string().email(),password:z.string().min(8).max(128)}).safeParse(req.body);
  if(!parsed.success)return res.status(400).json({error:'INVALID_INPUT'});
  try{
    const passwordHash=await bcrypt.hash(parsed.data.password,12);
    const created=await tx(async c=>{
      const u=await c.query(`INSERT INTO users(email,password_hash,name,role) VALUES(lower($1),$2,$3,'CLIENTE') RETURNING id,email,name,role`,[parsed.data.email,passwordHash,parsed.data.name]);
      await c.query('INSERT INTO wallets(user_id,balance) VALUES($1,0)',[u.rows[0].id]);
      await c.query('INSERT INTO audit_logs(user_id,action,entity,entity_id) VALUES($1,$2,$3,$4)',[u.rows[0].id,'REGISTER','USER',u.rows[0].id]);
      return u.rows[0];
    });
    res.status(201).json({token:signToken(created),user:created});
  }catch(error:any){
    if(error?.code==='23505')return res.status(409).json({error:'EMAIL_ALREADY_EXISTS'});
    throw error;
  }
});
...
api.post('/auth/login',async(req,res)=>{
  ...
  if(!q.rowCount || !(await bcrypt.compare(parsed.data.password,q.rows[0].password_hash)))return res.status(401).json({error:'INVALID_CREDENTIALS'});
  const u=q.rows[0]; const user={id:u.id,email:u.email,name:u.name,role:u.role};
...
api.get('/query-products',auth,async(_req,res)=>{
  const q=await pool.query('SELECT id,name,description,credit_cost FROM query_products WHERE active=true ORDER BY credit_cost');res.json(q.rows);
});
api.post('/queries',auth,async(req,res)=>{
  ...
  if(!/^[A-Z]{3}[0-9][A-Z0-9][0-9]{2}$/.test(plate))return res.status(400).json({error:'INVALID_PLATE'});
  const provider=getProvider(); let queryId=''; let cost=0;
  try{
    await tx(async c=>{
      const p=await c.query('SELECT credit_cost FROM query_products WHERE id=$1 AND active=true',[parsed.data.productId]);
      if(!p.rowCount)throw Object.assign(new Error('PRODUCT_NOT_FOUND'),{http:404});
      cost=p.rows[0].credit_cost;
      const w=await c.query('SELECT balance FROM wallets WHERE user_id=$1 FOR UPDATE',[req.user!.id]);
      if(!w.rowCount || w.rows[0].balance<cost)throw Object.assign(new Error('INSUFFICIENT_CREDITS'),{http:402});
    ...
    const out=await provider.queryByPlate(plate); const normalized=normalizeBdrp(out.raw);
    await tx(async c=>{
    ...
    if(queryId){await tx(async c=>{const q=await c.query('SELECT status FROM vehicle_queries WHERE id=$1 FOR UPDATE',[queryId]);if(q.rowCount&&q.rows[0].status==='PROCESSING'){const w=await c.query('SELECT balance FROM wallets WHERE user_id=$1 FOR UPDATE',[req.user!.id]);const before=w.rows[0].balance,after=before+cost;await c.query('UPDATE wallets SET balance=$2,updated_at=now() WHERE user_id=$1',[req.user!.id,after]);await c.query(`INSERT INTO wallet_transactions(user_id,kind,amount,balance_before,balance_after,query_id,description) VALUES($1,'REFUND',$2,$3,$4,$5,$6)`,[req.user!.id,cost,before,after,queryId,`Estorno consulta ${plate}`]);await c.query(`UPDATE vehicle_queries SET status='REFUNDED',error_code=$2,error_message=$3,completed_at=now() WHERE id=$1`,[queryId,error.code??'PROVIDER_ERROR',String(error.message).slice(0,250)]);}});}
...
api.post('/payments/sandbox',auth,async(req,res)=>{
  if(env.NODE_ENV==='production'&&!env.SANDBOX_CREDIT_PURCHASE_ENABLED)return res.status(403).json({error:'SANDBOX_DISABLED_IN_PRODUCTION'});
  const parsed=z.object({credits:z.number().int().min(10).max(10000)}).safeParse(req.body);if(!parsed.success)return res.status(400).json({error:'INVALID_INPUT'});
...
app.use('/api',api);
app.use((err:any,_req:express.Request,res:express.Response,_next:express.NextFunction)=>{console.error(err);res.status(500).json({error:'INTERNAL_ERROR'});});

const __dirname=path.dirname(fileURLToPath(import.meta.url));
const webDist=path.resolve(__dirname,'../../web/dist');
app.use(express.static(webDist,{index:false,maxAge:env.NODE_ENV==='production'?'1h':0}));
app.get('*',(req,res,next)=>{if(req.path.startsWith('/api/')||req.path==='/health')return next();res.sendFile(path.join(webDist,'index.html'));});
