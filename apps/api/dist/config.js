...
function requireProduction(condition, message, issues) {
    if (!condition)
        issues.push(message);
}
...
function validateProductionConfig(value) {
    const issues = [];
    ...
    if (issues.length > 0)
        throw new Error(`PRODUCTION_CONFIGURATION_INVALID:${issues.join(',')}`);
}
