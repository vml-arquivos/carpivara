...
function queryValue(request, names) {
...
function paymentIdFromRequest(request, payload) {
...
export class MercadoPagoProvider {
    ...
    async createCheckout(input) {
        ...
        const response = await requestJson(`${MERCADO_PAGO_API}/checkout/preferences`, {
            method: 'POST',
            headers: {
        ...
        if (!response.ok || !response.body.id || !link)
            throw codedError('PAYMENT_PROVIDER_REQUEST_FAILED');
    ...
    parseWebhookEvent(payload, query) {
    ...
    async fetchPaymentStatus(externalPaymentId) {
        const response = await requestJson(`${MERCADO_PAGO_API}/v1/payments/${encodeURIComponent(externalPaymentId)}`, {
            headers: { accept: 'application/json', authorization: `Bearer ${this.accessToken()}` }
        });
        if (!response.ok || typeof response.body.status !== 'string')
            throw codedError('PAYMENT_PROVIDER_REQUEST_FAILED');
